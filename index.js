...
function getPosts(flair, sortBy, timeFrame) {
  reddit.search(flair, sortBy, timeFrame).then(posts => {
    let postList = [];
    let count = 0;
    posts.forEach(post => {
      reddit.getComments(post.id)
        .then(comments => {
          postList.push([post.id, post.title, comments, post.selftext, post.score, post.url, "", 0]);
          count++;
          if (count === 100) {
...
function frequentTicker(post) {
  ...
  // var commentWords = "";
  // post[2].forEach(comment => {
  //   commentWords += " ";
  //   commentWords += comment.body;
  // })
