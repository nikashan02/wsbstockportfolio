...
function getCurrentStock(ticker) {
  return fetch(`https://api.tiingo.com/tiingo/daily/${ticker}/prices?token=3a854495ce1e479741bccc9fd27dccd3bd6ea479`)
    .then(res => res.json())
    .then(data => data.close)
    .catch(err => console.log(err))
}

function getHistoricalStock(ticker, startDate, endDate) {
  return fetch(`https://api.tiingo.com/tiingo/daily/${ticker}/prices?startDate=${startDate}&endDate=${endDate}&token=3a854495ce1e479741bccc9fd27dccd3bd6ea479`)
    .then(res => res.json())
...
app.get("/getPostList", (req, res) => {
  search(flair, sortBy, timeFrame).then(posts => {
    ...
    posts.forEach(post => {
      getComments(post.id)
        .then(comments => {
          postList.push([post.id, post.title, comments, post.selftext, post.score, post.url, "", 0, epochToDate(post.created), 0]); // historical price
          count++;
          if (count === 100) {
            ...
            for (var i=0; i<postList.length; i++) {
              ...
              var startDate = `${dateList[0]}-${dateList[1]}-${parseInt(dateList[2])-2}`;
              var endDate = `${dateList[0]}-${dateList[1]}-${dateList[2]}`;

              getHistoricalStock(postList[i][6], startDate, endDate)
                .then(historical => {
                  postList[i][9] = historical;
                  if (i===postList.length-1) {
                    var tickerList = [[postList[0][6], postList[0][4], 0, 0, 0]];
                    ...
                    for (var x=1; x<postList.length; x++) {
                      if (postList[x][6] !== "") {
                        ...
                        if (!found) {
                          tickerList.push([postList[x][6], postList[x][4], 0, 0, 0]);
                        }
                      }
                    }
                    ...
                    for (var x=0; x<tickerList.length; x++) {
                      getCurrentStock(tickerList[x][0])
                        .then(currentStock => {
                          tickerList[y][4] = currentStock;
      ...
      .then(comments => {
        ...
        if (count === 100) {
          ...
          for (var i=0; i<postList.length; i++) {
            ...
            var endDate = `${dateList[0]}-${dateList[1]}-${dateList[2]}`;
            console.log(i);
            const promise = getHistoricalStock(postList[i][6], startDate, endDate)
              .then(historical => {
                console.log(i);
                postList[i][9] = historical;
              });
          ...
          Promise.all(promises)
            .then(() => {
                ...
                for (var x=0; x<tickerList.length; x++) {
                  const promise2 = getCurrentStock(tickerList[x][0])
                    .then(currentStock => {
                      tickerList[y][4] = currentStock;
                    });
